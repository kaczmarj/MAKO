"""Use Phikon-v2 to extract latent features from whole slide images."""
...
def main(
    *,
    ...
    print("-" * 40)
    print("Extracting features with Phikon-v2")
    print("=" * 40)
    print("Arguments")
    print(f"  {wsi_dir=}")
    print(f"  {patch_dir=}")
    ...
    for wsi_path, patch_path in tqdm(
        ...
        for batch in tqdm(loader, leave=False, desc="Patches"):
            ...
            with torch.inference_mode():
                embedding = model(pixel_values=batch).last_hidden_state[:, 0, :]
                embeddings.append(embedding.detach().cpu())

        embeddings_tensor = torch.cat(embeddings)
        del embeddings
        assert embeddings_tensor.shape == (len(dset), 1024)
