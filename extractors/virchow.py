"""Use Virchow to extract latent features from whole slide images."""
...
def main(
    ...
    print("-" * 40)
    print("Extracting features with Virchow")
    print("=" * 40)
    print("Arguments")
    print(f"  {wsi_dir=}")
    print(f"  {patch_dir=}")
    ...
    for wsi_path, patch_path in tqdm(
        ...
        for batch in tqdm(loader, leave=False, desc="Patches"):
            ...
            with (
                ...
                output = model(batch)  # size: N x 257 x 1280
            class_token = output[:, 0]  # size: N x 1280
            patch_tokens = output[:, 1:]  # size: N x 256 x 1280
            # concatenate class token and average pool of patch tokens
            embedding = (
                torch.cat([class_token, patch_tokens.mean(1)], dim=-1).detach().cpu()
            )  # size: N x 2560
