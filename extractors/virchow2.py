"""Use Virchow2 to extract latent features from whole slide images."""
...
def main(
    ...
    print("-" * 40)
    print("Extracting features with Virchow2")
    print("=" * 40)
    print("Arguments")
    print(f"  {wsi_dir=}")
    print(f"  {patch_dir=}")
    ...
    for wsi_path, patch_path in tqdm(
        ...
        for batch in tqdm(loader, leave=False, desc="Patches"):
            ...
            with (
                ...
                output = model(batch)  # size: 1 x 261 x 1280
            ...
            assert embedding.shape == (len(batch), 2560)
            embeddings.append(embedding)
        embeddings_tensor = torch.cat(embeddings)
        del embeddings
        assert embeddings_tensor.shape == (len(dset), 2560)
