"""Use UNI2-h to extract latent features from whole slide images."""
...
def main(
    ...
    print("-" * 40)
    print("Extracting features with UNI2-h")
    print("=" * 40)
    print("Arguments")
    print(f"  {wsi_dir=}")
    print(f"  {patch_dir=}")
    ...
    model.eval()
    print(f"Placing model on device {device}...")
    model.to(device)
    ...
    for wsi_path, patch_path in tqdm(
        ...
        with torch.inference_mode():
            for batch in tqdm(loader, leave=False, desc="Patches"):
                embedding = model(batch.to(device))
                assert embedding.shape == (len(batch), 1536), embedding.shape
                embeddings.append(embedding.cpu())
        embeddings_tensor = torch.cat(embeddings)
        del embeddings
        assert embeddings_tensor.shape == (len(dset), 1536)
